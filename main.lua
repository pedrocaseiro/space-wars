...
function love.load()
  ...
  coolSmallFont = love.graphics.newFont("ProggySquareTT.ttf", 20)
	...
	if currentState == "game" then
		...
		if love.mouse.isDown("l") then
			...
			if player.canShoot then
				--a cada bala carrego o som da bala e reproduzo
				...
				newBullet = {
					x=player.x,
					y=player.y,
					dirX = player.bulletSpeed * math.cos(angle),
					dirY = player.bulletSpeed * math.sin(angle)
				}
		...
		else if currentState == "gameover" then
...
function love.draw()
	...
	if currentState == "game" then
		...
		love.graphics.draw(playerImage, player.x, player.y, math.atan2(love.mouse.getY() - player.y, love.mouse.getX() - player.x), 1, 1, playerImage:getWidth() / 2, playerImage:getHeight() / 2)


		--ciclo para desenhar as balas
		for i=1, #bullets do
			love.graphics.draw(bulletImage, bullets[i].x, bullets[i].y)
		end

		for i=1, #enemies do
			love.graphics.draw(enemyImage, enemies[i].x, enemies[i].y, math.atan2(player.y - enemies[i].y, player.x - enemies[i].x) + math.pi, 1, 1, playerImage:getWidth() / 2, playerImage:getHeight() / 2)
		end

		love.graphics.print(player.score, 20, 20)
		love.graphics.print(player.vidas, 20, 600)

	elseif currentState == "gameover" then
  		love.graphics.setFont(coolSmallFont)
		love.graphics.print("Game Over, press k to start again.", 250, 320)
		love.graphics.print("Your score was: " .. player.score, 250, 350)
	end
end
